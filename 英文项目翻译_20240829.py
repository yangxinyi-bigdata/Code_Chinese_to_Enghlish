...
def 代码文件翻译_V1(代码相对路径, 项目_根路径, 项目_文件表单, 项目_目录表单):
    代码_绝对路径 = 项目_根路径 + "/" + 代码相对路径
    日志.debug("当前处理 代码_绝对路径: {}", 代码_绝对路径)
    ...
    for 导入变量 in 变量列表_key:
        ...
        if 点数量 > 0:  # 如果前边有点, 则肯定是项目内部的
            ...
            if 代码所在目录 + 导入路径 in 项目_目录表单.相对路径.values:  # 同文件夹包
                处理导入(导入变量_去点)
            elif 代码所在目录 + 导入路径 + ".py" in 项目_文件表单.英文相对路径.values:  # 同文件夹py代码
                处理导入(导入变量_去点)
    ...
    if 文件名前缀 not in 英文变量字典:
        ...
        # 将翻译的结果也保存到变量中
        英文变量字典.setdefault(文件名前缀, 文件名前缀_翻译字典[文件名前缀])
...
def 复制项目代码翻译(项目_根路径):
    ...
    项目_文件表单 = pd.DataFrame(columns=["文件名称", "中文变量", "英文相对路径",
                                          "中文相对路径", "英文上层路径", "中文上层路径", "修改标识", "翻译文件"])
    ...
    for 索引 in 项目_文件表单.index:
        ...
        if 项目_文件表单.loc[索引, "翻译文件"] == 1:
            代码文件翻译_V1(代码相对路径, 项目_根路径, 项目_文件表单, 项目_目录表单)
            # 删除原来文件
            os.remove(项目_根路径 + "/" + 代码相对路径)
