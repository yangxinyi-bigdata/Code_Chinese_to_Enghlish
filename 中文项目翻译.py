...
def 创建英文文件夹(项目_根路径):
    ...
    for 完整路径 in 收集_相对目录列表:
        if 完整路径:  # 排除根路径
            上层路径, 文件夹名称 = 完整路径.rsplit("/", maxsplit=1)
            # 0 对应 英文变量
            # 1 对应 完整路径
            # 2 对应 完整路径 后续翻译成英文
            # 3 对应上层路径
    ...
    for 文件夹名称 in 项目_中文目录映射:
        项目_中文目录映射[文件夹名称]["英文变量"] = 合并后变量[文件夹名称]

    for 文件夹名称 in 项目_中文目录映射:

        if not 项目_中文目录映射[文件夹名称]["修改标识"]:  # 这个文件夹还没有被替换过路径, 就是原生路径
            # 直接创建该文件夹, 如果没有修改标识, 代表这个目录的上层都是英文的,
            英文上层路径 = 项目_中文目录映射[文件夹名称]["英文上层路径"]
            创建目录 = 项目_根路径 + "/" + 英文上层路径 + "/" + 项目_中文目录映射[文件夹名称]["英文变量"]
            原目录 = 项目_根路径 + "/" + 英文上层路径 + "/" + 文件夹名称
            项目_中文目录映射[文件夹名称]["英文完整路径"] = 项目_中文目录映射[文件夹名称]["英文完整路径"].replace(文件夹名称,
                                                                                        合并后变量[文件夹名称])

            if not os.path.exists(创建目录):
                shutil.copytree(原目录, 创建目录)
            # todo: 这里可能需要处理, 如果这个文件夹已经存在了又怎么办呢? 例如又一个 工具 还有一个 tools, 那么最终应该将这两个目录合并起来

            # 这里要对所有该目录的子目录的 "英文完整路径", "英文上层路径" 进行修改
            for 目录 in 项目_中文目录映射:  #
                if 项目_中文目录映射[目录]["完整路径"] != 项目_中文目录映射[文件夹名称]["完整路径"]:  # 如果不是当前项目
                    if 项目_中文目录映射[目录]["英文完整路径"].find(项目_中文目录映射[文件夹名称]["完整路径"]) > -1:
                        # 上层目录修改过, 标记一下
                        项目_中文目录映射[目录]["修改标识"] = 1
                        项目_中文目录映射[目录]["英文完整路径"] = 项目_中文目录映射[目录]["英文完整路径"].replace(
                            项目_中文目录映射[文件夹名称]["完整路径"], 项目_中文目录映射[文件夹名称]["英文完整路径"])
                        项目_中文目录映射[目录]["英文上层路径"] = 项目_中文目录映射[目录]["英文上层路径"].replace(
                            项目_中文目录映射[文件夹名称]["完整路径"], 项目_中文目录映射[文件夹名称]["英文完整路径"])
            # 将原文件夹中的所有内容复制过来
        else:  # 说明不是第一层文件夹
            ...
            for 目录 in 项目_中文目录映射:  #
                if 项目_中文目录映射[目录]["完整路径"] != 项目_中文目录映射[文件夹名称]["完整路径"]:  # 如果不是当前项目
                    if 项目_中文目录映射[目录]["完整路径"].find(项目_中文目录映射[文件夹名称]["完整路径"]) > -1:
                        # 上层目录修改过, 标记一下
                        项目_中文目录映射[目录]["修改标识"] += 1
                        项目_中文目录映射[目录]["英文完整路径"] = 项目_中文目录映射[目录]["完整路径"].replace(
                            项目_中文目录映射[文件夹名称]["完整路径"], 项目_中文目录映射[文件夹名称]["英文完整路径"])
                        项目_中文目录映射[目录]["英文上层路径"] = 项目_中文目录映射[目录]["上层路径"].replace(
                            项目_中文目录映射[文件夹名称]["完整路径"], 项目_中文目录映射[文件夹名称]["英文完整路径"])
...
if __name__ == '__main__':
    ...
    # 日志.info(收集_文件列表)
    # 关联文件判断(project_root)
    项目_中文目录映射 = 创建英文文件夹(项目_根路径)
    日志.info(项目_中文目录映射)

    收集_目录列表, 收集_文件列表 = 遍历目录和文件列表(项目_根路径)
    日志.info(收集_文件列表)
    # 过滤包含中文的py文件, 首先应该将将文件名称提取出来.
    项目_文件映射 = {}
    项目_中文文件映射 = {}
    for 完整路径 in 收集_文件列表:
        上层路径, 文件名称 = 完整路径.rsplit("/", maxsplit=1)
        # 当有两个文件名称都是 测试代码.py的时候, 就会导致后面的将前面的覆盖掉
        #
        项目_文件映射[文件名称] = {"英文变量": "",
                                     "完整路径": 完整路径,
                                     "英文完整路径": 完整路径,
    ...
    # 1. 提取出 "完整路径" 序列, 如果在这里, 就不处理
    # 2. 提取出 "'英文完整路径'" 序列, 如果在这里, 就替换
    # 3. 如果不在这两个当中, 就是原生的, 创建一个新的英文版本, 原来的保留.

    完整路径_列表 = [项目_中文目录映射[中文目录]["完整路径"] for 中文目录 in 项目_中文目录映射]
    英文完整路径_列表 = [项目_中文目录映射[中文目录]["英文完整路径"] for 中文目录 in 项目_中文目录映射]

    for py文件 in 项目_中文文件映射:
        if 项目_中文文件映射[py文件]["上层路径"] in 完整路径_列表:
            pass  # 不用处理
        elif 项目_中文文件映射[py文件]["英文上层路径"] in 英文完整路径_列表:
            代码文件翻译_V2(项目_中文文件映射[py文件]["英文完整路径"])  # 翻译成英文
            # 删除对应的中文
            os.remove(项目_中文文件映射[py文件]["完整路径"])
        else:  # 不在两个当中, 应该是翻译, 但是原来的保留.
            代码文件翻译_V2(项目_中文文件映射[py文件]["完整路径"])  # 翻译成英文
