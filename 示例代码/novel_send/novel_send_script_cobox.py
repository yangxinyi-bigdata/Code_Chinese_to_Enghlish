...
class NovelHandle:
    ...
    def read_noval_content(self):
        ...
        with open(self.input_file_path, "r", encoding="gbk") as f:
    ...
    def merge_6_chapters(self, merge_chapter_nums):
        """更新函数功能:
...
if __name__ == '__main__':
    ...
    novel_handle = NovelHandle("宿命之环", input_file_path="./原文件/宿命之环.txt",
                               output_dir="./处理后")
    ...
    # 合并当天6章内容, 并返回合并文件路径
    save_merge_file_name = novel_handle.merge_6_chapters(6)
    # 通过文本路径读取6章内容
    with open(save_merge_file_name) as f:
        content = f.read()

    # 发送邮件
    qq_email_send = QQEmailSend(sender_email = "yangyuehaha@qq.com",
                                password = "yunkrcylslptcada",   # 使用QQ邮箱提供的授权码，而不是你的QQ密码,
            receiver_email = "yangyuegaga@qq.com") # "yangyuehaha_kindle@kindle.cn"
    qq_email_send.create_message("小说发送", body=content[0:1000], filename=save_merge_file_name)
