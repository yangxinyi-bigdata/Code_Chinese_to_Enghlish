...
def 代码文件翻译_V1(代码_绝对路径, 项目_根路径):
    ...
    for 导入变量 in 变量列表_key:
        ...
        if 点数量 > 0:  # 如果前边有点, 则肯定是项目内部的
            ...
            if 导入变量_去点.count(".") > 0:
                if 代码相对目录 + 导入路径 in 收集_相对目录列表:  # 同文件夹包
                    变量列表2_项目内.extend(导入变量_去点.split("."))
                elif 代码相对目录 + 导入路径 + ".py" in 收集_相对文件列表:  # 同文件夹py代码
                    变量列表2_项目内.extend(导入变量_去点.split("."))
                elif 导入路径 in 收集_相对目录列表:  # 根文件夹包
                    变量列表2_项目内.extend(导入变量_去点.split("."))
                elif 导入路径 + ".py" in 收集_相对文件列表:  # 根py代码
                    变量列表2_项目内.extend(导入变量_去点.split("."))
                else:  # 都没找到
                    找到路径 = False
            else:
        ...
        # for 后面变量 in 变量列表_key[导入变量]:
        #     变量列表2_项目内.append(后面变量)
            # if 代码相对目录 + 导入路径 + "/" + 后面变量 in 收集_相对目录列表:  # 同文件夹包
            #     变量列表2_项目内.append(导入变量)
            # elif 代码相对目录 + 导入路径 + "/" + 后面变量 + ".py" in 收集_相对文件列表:  # 同文件夹py代码
            #     变量列表2_项目内.append(导入变量)
            # elif 导入路径 + "/" + 后面变量 in 收集_相对目录列表:  # 根文件夹包
            #     变量列表2_项目内.append(导入变量)
            # elif 导入路径 + "/" + 后面变量 + ".py" in 收集_相对文件列表:  # 根py代码
            #     变量列表2_项目内.append(导入变量)
    ...
    变量列表.extend(变量列表3)
    变量列表_列表 = list(set(变量列表))

    # 获取了所有变量集合, 下一步应该是翻译成英文
    # 提取需要翻译的变量, 例如 os , 找不到这个变量, 那么这个变量就过滤掉.
    ...
    英文变量映射 = {变量: 合并后变量_字典[变量] for 变量 in 变量列表_列表}
    # 替换只需要替换 `中文变量列表` 中的就可以了, 需要一个映射关系
    代码文本 = 函数_替换代码变量(代码文本, 英文变量映射)
...
def 转换英文文件夹_到中文(项目_根路径):
    ...
    # 合并后变量_字典 = 查询英文变量(变量列表_列表)
    合并后变量 = 查询英文变量(项目_纯英文目录表单["文件夹名称"].to_list())

    for 索引 in 项目_纯英文目录表单.index:
        项目_纯英文目录表单.loc[索引, "中文变量"] = 合并后变量[项目_纯英文目录表单.loc[索引, "文件夹名称"]]

    for 索引 in 项目_纯英文目录表单.index:
        # 直接创建该文件夹, 如果没有修改标识, 代表这个目录的上层都是英文的, 或者是第一层文件夹
        文件夹名称 = 项目_纯英文目录表单.loc[索引, "文件夹名称"]
        中文上层路径 = 项目_纯英文目录表单.loc[索引, "中文上层路径"]
        创建目录 = 项目_根路径 + "/" + 中文上层路径 + "/" + 项目_纯英文目录表单.loc[索引, "中文变量"]
        原目录 = 项目_根路径 + "/" + 中文上层路径 + "/" + 文件夹名称
        # 这里有可能并没有成功翻译成中文, 所以中文变量, 和原来的英文变量是一样的, 导致报错
        项目_纯英文目录表单.loc[索引, "中文相对路径"] = 项目_纯英文目录表单.loc[索引, "中文相对路径"].replace(
            文件夹名称,
            合并后变量[文件夹名称])
        ...
        # for 索引_内层 in 项目_纯英文目录表单.index:
        #     if 索引 != 索引_内层:  # 如果不是当前处理目录
        #         if 判断子目录(项目_纯英文目录表单.loc[索引, "相对路径"], 项目_纯英文目录表单.loc[索引_内层, "相对路径"]):
        #             # 上层目录修改过, 标记一下
        #             项目_纯英文目录表单.loc[索引_内层, "修改标识"] = 1  # 没用了
        #             项目_纯英文目录表单.loc[索引_内层, "中文相对路径"] = 项目_纯英文目录表单.loc[
        #                 索引_内层, "相对路径"].replace(
        #                 项目_纯英文目录表单.loc[索引, "相对路径"], 项目_纯英文目录表单.loc[索引, "中文相对路径"])
        #
        #             项目_纯英文目录表单.loc[索引_内层, "中文上层路径"] = 项目_纯英文目录表单.loc[
        #                 索引_内层, "上层路径"].replace(
        #                 项目_纯英文目录表单.loc[索引, "相对路径"], 项目_纯英文目录表单.loc[索引, "中文相对路径"])
    ...
    # for 索引 in 项目_中文目录表单:
        # 对于中文目录表单应该如何处理? 要判断上级是否存在英文目录, 要知道当前已经被翻译成了中文目录, 所以对应路径也应该修改
...
if __name__ == '__main__':
    ...
    # 日志.info(项目_文件表单)
    # 日志.info(英文相对路径_列表)
    项目_翻译路径 = "/Users/yangxinyi/Downloads/200_临时文件夹/英文测试项目"
