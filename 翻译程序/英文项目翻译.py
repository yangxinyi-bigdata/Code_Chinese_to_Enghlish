...
def 全项目代码翻译(项目_根路径):
    ...
    for 索引 in 项目_中文文件表单.index:
        ...
        if 上层路径 == "":  # 项目根路径中, 直接翻译
            英文代码_保存路径 = 英文代码文件翻译(项目_根路径 + "/" + 项目_中文文件表单.loc[索引, "相对路径"])  # 翻译成英文
...
def 开始翻译_英文文件(文件路径):
    ...
    设置状态值("处理重复变量...")
    设置进度值(20)
    rope单文件管理.提取变量表格 = rope单文件管理.处理重复变量(rope单文件管理.提取变量表格)
    设置状态值("开始替换变量...")
    设置进度值(30)
    rope单文件管理.批量替换变量()
    日志.info("翻译生成文件: {}", rope单文件管理.我的资源.real_path)
    rope单文件管理.关闭项目()
    设置状态值("翻译完成")
    设置进度值(100)


def 开始翻译_jupyter英文文件翻译(代码_绝对路径):
    """用于手工(非Rope)方式对代码Py文件和ipynb进行翻译.
    ...
    with open(代码_绝对路径, 'r', encoding='utf-8') as file:
        notebook = json.load(file)
        # 代码文本 = ipynb转换成py(notebook)
        代码文本 = 提取jupyter代码(notebook)
    # 保存代码文本
    转换py代码保存路径 = 代码_绝对路径.removesuffix("ipynb") + "py"
    with open(转换py代码保存路径, "w") as f:
        f.write(代码文本)
    # 这里可以使用Rope提取变量, 首先应该是复制了一个新的, 然后将新的路径创建Rope项目, 和资源.然后提取变量
    # 不管是py和ipynb都可以用Rope提取变量, 但是py直接扔进去就可以. ipynb需要处理
    # 将转换的ipynb代码保存到一个临时路径,
    # 创建项目
    设置状态值("开始提取变量")
    设置进度值(20)
    rope单文件管理 = Rope单文件管理器(转换py代码保存路径)

    rope单文件管理.创建资源(转换py代码保存路径)
    rope单文件管理.获取监控范围()
    rope单文件管理.提取范围变量()
    设置状态值("开始翻译变量")
    设置进度值(20)
    ...
    rope单文件管理.批量替换变量()
    # 重新读取代码
    代码文本_翻译后 = rope单文件管理.我的资源.read()
    # 将翻译完成的代码保存回原来的notebook中

    替换jupyter代码(notebook, 代码文本_翻译后)

    # 形成变量映射字典
    # 变量映射字典 = rope单文件管理.提取变量表格.set_index("变量名称")["翻译变量"].to_dict()
    os.remove(转换py代码保存路径)
    设置状态值("开始保存翻译文件")
    设置进度值(90)
    # 翻译文件名, 创建翻译文件
    上层文件夹, 文件名称 = 代码_绝对路径.rsplit("/", maxsplit=1)
    文件前缀, 文件后缀 = 文件名称.rsplit(".", maxsplit=1)
    翻译文件名映射字典 = 过滤并查询英文变量_V2([文件前缀])
    if 翻译文件名映射字典.get(文件前缀):
        翻译文件名称 = 翻译文件名映射字典[文件前缀] + "." + 文件后缀
        翻译文件路径 = 上层文件夹 + "/" + 翻译文件名称
    else:  # 没翻译到结果, 直接复制一个添加后缀的
        翻译文件路径 = 上层文件夹 + "/" + 文件前缀 + "_翻译" + "." + 文件后缀
    日志.debug("翻译文件路径: {}", )

    with open(翻译文件路径, "w") as f:
        json.dump(notebook, f, ensure_ascii=False, indent=4)
    设置状态值("翻译完成")
    设置进度值(100)
    return 翻译文件路径
...
if __name__ == '__main__':
    ...
    path = "/Users/yangxinyi/Library/Mobile Documents/com~apple~CloudDocs/100-CodeEngine/350-数据分析完善案例 - 高价值/20200209 - 基于欺诈检测的综合财务数据集预测金融支付服务中的欺诈行为/Predicting Fraud in Financial Payment Services.ipynb"
    开始翻译_jupyter英文文件翻译(path)
